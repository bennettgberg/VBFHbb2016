...
#Trying to make Bias Templates without using transfer functions.            #BG comment
...
#	mg2.add_option('--TF',help=colours[5]+'Transfer function label: POL1,POL1 (Single,Double)'+colours[0],default=['POL2','POL4'],type='str',action='callback',callback=optsplit) #BG comment
...
#	fTRF = TFile.Open("%s/root/TransferFunctions%s.root"%(opts.workdir,"" if not opts.long else longtag1),"read") #BG comment
	wBKG = fBKG.Get("w")
	wSIG = fSIG.Get("w")
...
# Selection loop
# meaning selsingle and then seldouble (or is it the other way around?)      #BG comment
...
#				brn_params[nbp].add(brn[nb])
  ## x
			...
			trans_p = {}
  ## Transfer functions			
#			if not C==0:                                                                              #BG comments
#				ftf = fTRF.Get("fRat_sel%s_CAT%d"%(S.label,Cp))                                   #||||||||||||
#				npar = ftf.GetNpar()                                                              #vvvvvvvvvvvv
#  ### TF Parameters
...
#  				transfer[ntf] = RooGenericPdf(ntf,fTF[opts.TF[iS]]['f'],rl)                     #End BG comments
...
#			if 1 == 0: #BG change, don't want this name
		...
		#where does qcd_model_aux come from ?????        #BG comment
				#if (opts.function.find('Pol')==-1) : qcd_pdf_aux[N] =  ROOT.RooGenericPdf("qcd_model_aux_%s_CAT%d"%(''.join(opts.TF),Cp),"",formula,params_aux) 
				if (opts.function.find('Pol')==-1) : qcd_pdf_aux[N] =  ROOT.RooGenericPdf("qcd_model_aux_CAT%d"%(Cp),"",formula,params_aux) #BG change
				...
				else : qcd_pdf_aux[N] = RooBernstein("qcd_model_aux_CAT%d"%(Cp),"qcd_model_aux_CAT%d"%(Cp),x,params_aux) #BG change
...
#				qcd_pdf_aux2[N] = RooProdPdf("qcd_model_CAT%d"%(Cp),"qcd_model_CAT%d"%(Cp),RooArgList(transfer["transfer_%s_CAT%d"%(opts.TF[iS],Cp)],qcd_pdf_aux[N])) #BG change-- what to use instead of transfer?? Just commented out qcd_pdf_aux2.
				#qcd_pdf[N]      = qcd_pdf_aux2[N] #RooAbsPdf(qcd_pdf_aux2[N])
				qcd_pdf[N]      = qcd_pdf_aux[N] 		#BG change
...
#			yS    = RooRealVar("yield_signal_mass%d_CAT%d"%(MASS,Cp),"yield_signal_mass%d_CAT%d"%(MASS,Cp),ySVBF.getValV()+ySGF.getValV(),0.,400.)
  ...
  ### Combined model
	  		model[N] = RooAddPdf("bkg_model_CAT%d"%(Cp),"bkg_model_CAT%d"%(Cp),RooArgList(zPDF[N],tPDF[N],qcd_pdf[N]),RooArgList(yZ[N],yT[N],yQ[N])) #BG change (this was commented before)
  ...
  #What does this mean ??? #BG comment
	#		for ib in range(opts.BRN[iS]+1):
	#			if C==0:
	#				nb = "b%d_CAT%d"%(ib,Cp)
...
#	w.writeToFile("%s/root/biasCATS_shapes_workspace_%s.root"%(opts.workdir,opts.function))
	w.writeToFile("%s/root/bias_shapes_workspace_BG_%s.root"%(opts.workdir,opts.function))
	print "Done."
####################################################################################################
